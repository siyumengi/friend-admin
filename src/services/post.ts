...
// 更新
export const getUpdatePost = async (postData) => {
    const resp= await myAxios.post("/post/update", postData);
...
export const getAllPost = async () => {
    let resp = await myAxios.get("/post/list" );
    if (resp.code === 0){
        return resp
    }
...
export const getDelete = async (id: number) => {
    const resp = await myAxios.post('/post/delete', {
